...
@property (readonly, nullable) NSArray *toArray;
...
- (nullable ObjectType)setObject:(ObjectType)anObject atIndexedSubscript:(NSUInteger)index;

/// 将指定对象插入到给定索引处，后面的元素全部往后挪动一位。
/// @return 如果插入成功则返回之前的旧对象，如果插入失败或插入在最后一位则返回nil。
- (nullable ObjectType)insertObject:(ObjectType)anObject atIndex:(NSUInteger)index;
